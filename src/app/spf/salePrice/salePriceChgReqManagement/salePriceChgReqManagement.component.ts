...
import { SalePriceChgReqManagement } from './shared/salePriceChgReqManagement.model';
import { SalePriceChgReqManagementService } from './shared/salePriceChgReqManagement.service';
...
@Component({
	selector: 'app-salePriceChgReqManagement',
	templateUrl: './salePriceChgReqManagement.component.html',
	providers: [SalePriceChgReqManagementService],
})
export class SalePriceChgReqManagementComponent implements OnInit {
	...
	//param model
	public paramModel: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};

	//grid select model
	public gridModel: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};
	public detailParamModel: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};
    //점
    public comboStoreLv1: any = null;
    public comboStoreLv1Model: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};

    //부
    public comboStoreLv2: any = null;
    public comboStoreLv2Model: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};

    //팀
    public comboStoreLv3: any = null;
    public comboStoreLv3Model: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};

	//grid store
	gridStore = new Ext.data.Store({});
	public chitList = new Array();
	public chitSearchModel: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};
	public chitResultModel: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};
	public chitGridModel: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};
	public gridParamModel: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};
	 //거래선체크
	 public checkPartner: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};
	//시작일
	public startDt: any = null;
	public chitYear: any = null;
	public saleChangeDt: any = null;
	//단품코드 리스트
	public itemList: any = null;
	//단품리스트 상세
	public itemDetail: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};
	public items: SalePriceChgReqManagement = <SalePriceChgReqManagement>{};
	...
	update6:any ;
	inq:any ;
	public saleChangeStore : any = [
		{saleChangeCd: '1', saleChangeNm:'1.인상'},
		{saleChangeCd: '2', saleChangeNm:'2.인하'}
	
		
	 ]
	constructor(private salePriceChgReqManagementService: SalePriceChgReqManagementService, public envService: EnvService,private cd: ChangeDetectorRef  ) { }
	ngOnInit() { 
		this.startDt = new Date(this.envService.getBeforeDate(30));
		this.endDt = new Date();
		this.saleChangeDt =new Date();
		...
		this.paramModel.saleChangeDt = this.envService.getDateToString(null);
		this.paramModel.saleChangeCd='';
	...
	//시작일 변경 이벤트
	onChangesaleChangeDt(date){
		console.log(date.newDate);
		this.paramModel.saleChangeDt = this.envService.getDateToString(date.newDate);
		this.saleChangeDt = date.newDate;
	}
	...
	onChangesaleChange(date){
		this.paramModel.saleChangeCd = date.newValue;
		console.log(this.paramModel.saleChangeCd);
	}
	//grid row 선택 이벤트
	onSelectGrid(row){
		//row 데이터 model 바인딩
	...
	onclear(){
		console.log(this.saleChangeDt);
		console.log(this.payDt);
	
		this.paramModel.saleChangeDt 	= this.envService.getDateToString(this.saleChangeDt);
	...
	onChitOk = () => {
		this.gridParamModel.itemsNm='';
			...
			this.salePriceChgReqManagementService.itemsCdCheck(this.gridParamModel).subscribe(
								(res: any) => {
										this.items=res;
			...
			if(this.items==null){Ext.Msg.alert('Error!!', '없는 품목코드입니다.');}
				...
				if(this.gridParamModel.cost==null||this.gridParamModel.cost==0){Ext.Msg.alert('Error!!', '원가를 입력하세요');}
				else{
					if(this.gridParamModel.price==null||this.gridParamModel.price==0){Ext.Msg.alert('Error!!', '매가를 입력하세요');}
						...
						if(this.gridParamModel.num==null||this.gridParamModel.num==0){Ext.Msg.alert('Error!!', '수량를 입력하세요');}
							...
							if(this.gridParamModel.itemsCd==null||this.gridParamModel.itemsCd==''){Ext.Msg.alert('Error!!', '품목코드를 입력하세요');}
								...
								totPrice:this.gridParamModel.num*this.gridParamModel.price
								});
								}
	...
	itemCdEvent = (data) =>{
		...
		this.salePriceChgReqManagementService.itemDetail(this.detailParamModel).subscribe(
			(res: any) => {
					this.itemDetail=res;
	...
	onTapQuery(event){
		...
		if(this.paramModel.cls==''){Ext.Msg.alert('Error!!', '클래스를 선택하세요.');}
			...
			if(this.paramModel.partner.substr(0,1)=='1'||this.paramModel.partner.substr(0,1)=='2'){
				if(this.paramModel.saleChangeCd==''){Ext.Msg.alert('Error!!', '변경구분을 선택하세요.');}
				else{
					console.log("구분성공");
					this.salePriceChgReqManagementService.checkPartner(this.paramModel).subscribe(
						(res: any) => {
						...
						if(this.checkPartner.partnerStatus!=''){
							...
							this.salePriceChgReqManagementService.itemList(this.paramModel).subscribe(
								(res: any) => {
										this.itemList=res;
										console.log(this.checkPartner);
								},
			...
			else{Ext.Msg.alert('Error!!', '매입인 경우에만 변경이 가능합니다.');}
   ...
   //삭제 버튼 이벤트
	onTapDelete(event){
		Ext.Msg.confirm("삭제", "데이터를 삭제합니다", 
			(e)=>{
				if( e == 'yes' ){
					console.log(this.paramModel);
					this.salePriceChgReqManagementService.deleteSalePriceChgReqManagement(this.paramModel).subscribe(
						(res: any) => {
							/**
							 * @success
							 */
	...
	//신규 버튼 이벤트
	onTapNew(event){
		// this.formModel = <SalePriceChgReqManagement>{};
	...
	onTapModify(event)	{
		...
		this.salePriceChgReqManagementService.updateSalePriceChgReqManagement(this.chitList).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
		...
		this.chitList=[];
		   console.log(this.paramModel);
			...
			for(let index in this.gridStore.getData().items) {
				...
				this.salePriceChgReqManagementService.insertSalePriceChgReqManagement(this.chitList).subscribe(
					(res: any) => {
						/**
						 * @success
						 */
	...
	//엑셀 버튼 이벤트
	onTapExcel(event){
		this.salePriceChgReqManagementService.excelSalePriceChgReqManagement(this.paramModel).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
	...
	//레포트 버튼 이벤트
	onTapReport(event){
		this.salePriceChgReqManagementService.reportSalePriceChgReqManagement(this.paramModel).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
	...
	//도움말 버튼 이벤트
	onTapHelp(event){
		this.salePriceChgReqManagementService.helpSalePriceChgReqManagement(this.paramModel).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
   ...
   onChitSearchOk = () => {
	...
	this.salePriceChgReqManagementService.chitSearchResult(this.chitSearchModel).subscribe(
		(res: any) => {
				...
				this.paramModel.saleChangeDt=this.chitResultModel.saleChangeDt;
				this.paramModel.saleChangeCd=this.chitResultModel.saleChangeCd;
				this.paramModel.saleChangeNm=this.chitResultModel.saleChangeNm;
				...
				this.salePriceChgReqManagementService.chitSearchResultDetail(this.chitSearchModel).subscribe(
					(res: any) => {
