...
import { AppSayPlusSale } 		from './shared/appSayPlusSale.model';
import { AppSayPlusSaleService } from './shared/appSayPlusSale.service';
import { EnvService } from '../../../shared/env.service';
import { HttpErrorResponse } from '@angular/common/http';

@Component({
	selector: 'app-appSayPlusSale',
	templateUrl: './appSayPlusSale.component.html',
	providers: [AppSayPlusSaleService],
})
export class AppSayPlusSaleComponent implements OnInit {

	@Input() public route: any;

	//param model
	public paramModel: AppSayPlusSale = <AppSayPlusSale>{};

	//grid select model
	public gridModel: AppSayPlusSale = <AppSayPlusSale>{};

   	//점
   	public comboStoreLv1: any = null;
   	public comboStoreLv1Model: AppSayPlusSale = <AppSayPlusSale>{};

   	//부
   	public comboStoreLv2: any = null;
   	public comboStoreLv2Model: AppSayPlusSale = <AppSayPlusSale>{};

   	//팀
   	public comboStoreLv3: any = null;
   	public comboStoreLv3Model: AppSayPlusSale = <AppSayPlusSale>{};
	...
	constructor(private appSayPlusSaleService: AppSayPlusSaleService, public envService: EnvService ) { }
	ngOnInit() { 
		//시작일자---
	...
	onTapQuery(event){
		...
		this.appSayPlusSaleService.selectAppSayPlusSaleList(this.paramModel).subscribe(
			(res: any) => {
				console.log(res);
				/**
				 * @success
	...
	onTapDelete(event){
		Ext.Msg.confirm("삭제", "데이터를 삭제합니다", 
			function(){
				this.appSayPlusSaleService.deleteAppSayPlusSale(null).subscribe(
					(res: any) => {
						/**
						 * @success
						 */
	...
	onTapSave(event){
		Ext.Msg.confirm("저장", "데이터를 저장합니다", 
			function(){
				this.appSayPlusSaleService.saveAppSayPlusSale(null).subscribe(
					(res: any) => {
						/**
						 * @success
						 */
	...
	//엑셀 버튼 이벤트
	onTapExcel(event){
		this.appSayPlusSaleService.excelAppSayPlusSale(this.paramModel).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
	...
	//레포트 버튼 이벤트
	onTapReport(event){
		this.appSayPlusSaleService.reportAppSayPlusSale(this.paramModel).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
	...
	//도움말 버튼 이벤트
	onTapHelp(event){
		this.appSayPlusSaleService.helpAppSayPlusSale(this.paramModel).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
