...
import { DirectoryOfPartnersInq } 		from './shared/directoryOfPartnersInq.model';
import { DirectoryOfPartnersInqService } from './shared/directoryOfPartnersInq.service';
import { EnvService } from '../../../shared/env.service';
import { HttpErrorResponse } from '@angular/common/http';

@Component({
	selector: 'app-directoryOfPartnersInq',
	templateUrl: './directoryOfPartnersInq.component.html',
	providers: [DirectoryOfPartnersInqService],
})
export class DirectoryOfPartnersInqComponent implements OnInit {
	...
	//param model
	public paramModel: DirectoryOfPartnersInq = <DirectoryOfPartnersInq>{};

	//grid select model
	public gridModel: DirectoryOfPartnersInq = <DirectoryOfPartnersInq>{};

   	//점
   	public comboStoreLv1: any = null;
   	public comboStoreLv1Model: DirectoryOfPartnersInq = <DirectoryOfPartnersInq>{};

   	//부
   	public comboStoreLv2: any = null;
   	public comboStoreLv2Model: DirectoryOfPartnersInq = <DirectoryOfPartnersInq>{};

   	//팀
   	public comboStoreLv3: any = null;
   	public comboStoreLv3Model: DirectoryOfPartnersInq = <DirectoryOfPartnersInq>{};
	...
	public comboOnStore : any = [
		{onlineChk: 'A', onlineChkNm: '상호명 입력'},
	]

	//comboBox Store
	public comboOnStoreNm : any = [
		{serchName: 'A', serchNameNm: '대표자 성명 입력'},
	]

	//comboBox Store
	public comboOnStoreCpNo : any = [
		{corpNo: 'A', corpNoNm: '사업자번호 입력'},
	]
	...
	constructor(private directoryOfPartnersInqService: DirectoryOfPartnersInqService, public envService: EnvService ) { }
	ngOnInit() { 
		//시작일자---
	...
	onTapQuery(event){
		...
		this.directoryOfPartnersInqService.selectDirectoryOfPartnersInqList(this.paramModel).subscribe(
			(res: any) => {
				console.log(res);
				/**
				 * @success
	...
	onTapDelete(event){
		Ext.Msg.confirm("삭제", "데이터를 삭제합니다", 
			function(){
				this.directoryOfPartnersInqService.deleteDirectoryOfPartnersInq(null).subscribe(
					(res: any) => {
						/**
						 * @success
						 */
	...
	onTapSave(event){
		Ext.Msg.confirm("저장", "데이터를 저장합니다", 
			function(){
				this.directoryOfPartnersInqService.saveDirectoryOfPartnersInq(null).subscribe(
					(res: any) => {
						/**
						 * @success
						 */
	...
	// //엑셀 버튼 이벤트
	onTapExcel(event){
		let url = this.directoryOfPartnersInqService.excelDirectoryOfPartnersInq(this.paramModel);
		console.log(url);
	...
	onTapPdfQuery(event){
		...
		this.pdfUrl = this.directoryOfPartnersInqService.reportDirectoryOfPartnersInq(this.paramModel);
	...
	//도움말 버튼 이벤트
	onTapHelp(event){
		this.directoryOfPartnersInqService.helpDirectoryOfPartnersInq(this.paramModel).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
...
//조직(점) 기본 셋팅
   ...
   //콤보박스 변경 이벤트
   onChangeComboBoxNm(data){
	console.log(`newValue : ${data.newValue} / oldValue : ${data.oldValue}`);
	this.paramModel.serchName = data.newValue;
}
   //콤보박스 변경 이벤트
   onChangeComboBoxCpNo(data){
	console.log(`newValue : ${data.newValue} / oldValue : ${data.oldValue}`);
	this.paramModel.corpNo = data.newValue;
}
}