...
import { BuyReqManagement } from './shared/buyReqManagement.model';
import { BuyReqManagementService } from './shared/buyReqManagement.service';
...
@Component({
	selector: 'app-buyReqManagement',
	templateUrl: './buyReqManagement.component.html',
	providers: [BuyReqManagementService],
})
export class BuyReqManagementComponent implements OnInit {
	...
	//param model
	public paramModel: BuyReqManagement = <BuyReqManagement>{};
	public uparamModel: BuyReqManagement = <BuyReqManagement>{};
	public chitSearchModel: BuyReqManagement = <BuyReqManagement>{};
	public chitResultModel: BuyReqManagement = <BuyReqManagement>{};
	
	public gridParamModel: BuyReqManagement = <BuyReqManagement>{};
	public detailParamModel: BuyReqManagement = <BuyReqManagement>{};

	//grid select model
	public gridModel: BuyReqManagement = <BuyReqManagement>{};
	public chitGridModel: BuyReqManagement = <BuyReqManagement>{};
	
    //점
    public comboStoreLv1: any = null;
    public comboStoreLv1Model: BuyReqManagement = <BuyReqManagement>{};

    //부
    public comboStoreLv2: any = null;
    public comboStoreLv2Model: BuyReqManagement = <BuyReqManagement>{};

    //팀
    public comboStoreLv3: any = null;
    public comboStoreLv3Model: BuyReqManagement = <BuyReqManagement>{};
	//단품코드 리스트
	public itemList: any = null;
	//단품리스트 상세
	public itemDetail: BuyReqManagement = <BuyReqManagement>{};
	//grid store
	gridStore = new Ext.data.Store({});

	//시작일
	public startDt: any = null;
	public orderDt: any = null;
	...
	public items: BuyReqManagement = <BuyReqManagement>{};
	public i: number;
	public j: number;
	public check: string;
	...
	inq:any ;
	 //거래선체크
	 public checkPartner: BuyReqManagement = <BuyReqManagement>{};
	

	constructor(private buyReqManagementService: BuyReqManagementService, public envService: EnvService,private cd: ChangeDetectorRef  ) { }
	ngOnInit() { 
		this.check='';
		...
		this.paramModel.orderDt 	= this.envService.getDateToString(null);
	...
	onChangeOrderDt(date){
		...
		this.paramModel.orderDt = this.envService.getDateToString(date.newDate);
		this.orderDt = date.newDate;
	}
	...
	onTapQuery(event){
		...
		if(this.paramModel.orderDt>this.paramModel.payDt){Ext.Msg.alert('Error!!', '발주일자는 납기일자 이후여야 합니다.');}
		else{	
					...
					if(this.paramModel.partner==''){Ext.Msg.alert('Error!!', '협력업체를 선택하세요.');}
							...
							if(
								...
								||((this.paramModel.partner.substr(0,1)=='5'||this.paramModel.partner.substr(0,1)=='7')&&(this.paramModel.cls.substr(4,1)=='5'||this.paramModel.cls.substr(4,1)=='7'))
									...
									this.buyReqManagementService.checkPartner(this.paramModel).subscribe(
										(res: any) => {
										...
										if(this.checkPartner.partnerStatus!=''){
											...
											this.buyReqManagementService.itemList(this.paramModel).subscribe(
												(res: any) => {
														this.itemList=res;
														console.log(this.checkPartner);
												},
	...
	onTapQueryu(event){
		...
		console.log(this.uparamModel);
		console.log(this.uparamModel.partner.substr(0,1))
		console.log(this.uparamModel.cls.substr(4,1))
		if(this.uparamModel.orderDt>this.uparamModel.payDt){Ext.Msg.alert('Error!!', '발주일자는 납기일자 이후여야 합니다.');}
		else{	
				if(this.uparamModel.cls==''){Ext.Msg.alert('Error!!', '클래스를 선택하세요.');}
				else{
					if(this.uparamModel.partner==''){Ext.Msg.alert('Error!!', '협력업체를 선택하세요.');}
					else{
							if(
								((this.uparamModel.partner.substr(0,1)=='1'||this.uparamModel.partner.substr(0,1)=='2')&&(this.uparamModel.cls.substr(4,1)=='1'||this.uparamModel.cls.substr(4,1)=='2'))
								||((this.uparamModel.partner.substr(0,1)=='3'||this.uparamModel.partner.substr(0,1)=='4')&&(this.uparamModel.cls.substr(4,1)=='3'||this.uparamModel.cls.substr(4,1)=='4'))
								||((this.uparamModel.partner.substr(0,1)=='5'||this.uparamModel.partner.substr(0,1)=='7')&&(this.uparamModel.cls.substr(4,1)=='5'||this.uparamModel.cls.substr(4,1)=='7'))
								)	{
									this.buyReqManagementService.checkPartner(this.uparamModel).subscribe(
										(res: any) => {
												this.checkPartner=res;
												console.log(this.checkPartner);
										},
										...
										if(this.checkPartner.partnerStatus!=''){
											...
											this.buyReqManagementService.itemList(this.uparamModel).subscribe(
												(res: any) => {
														this.itemList=res;
														console.log(this.checkPartner);
												},
   ...
   //삭제 버튼 이벤트
	onTapDelete(event){
		...
		(e)=>{
			if( e == 'yes' ){
			console.log(this.paramModel);
			this.buyReqManagementService.deleteBuyReqManagement(this.paramModel).subscribe(
				(res: any) => {
					/**
					 * @success
					 */
	...
	onTapModify(event){
		...
		this.buyReqManagementService.updateBuyReqManagement(this.chitList).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
	...
	onTapSave(event){
		console.log(this.paramModel);
					...
					// this.buyReqManagementService.insertBuyReqManagement(this.paramModel).subscribe(
					// 				(res: any) => {
					// 						this.i=res;
					...
					// 			);
								...
								for(let index in this.gridStore.getData().items) {
										...
										this.buyReqManagementService.insertBuyReqManagement2(this.chitList).subscribe(
											(res: any) => {
												/**
												 * @success
												 */
	...
	onclear(){
		...
		this.paramModel.orderDt 	= this.envService.getDateToString(this.orderDt);
	...
	//엑셀 버튼 이벤트
	onTapExcel(event){
		this.buyReqManagementService.excelBuyReqManagement(this.paramModel).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
	...
	//레포트 버튼 이벤트
	onTapReport(event){
		this.buyReqManagementService.reportBuyReqManagement(this.paramModel).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
	...
	//도움말 버튼 이벤트
	onTapHelp(event){
		this.buyReqManagementService.helpBuyReqManagement(this.paramModel).subscribe(
			(res: any) => {
				/**
				 * @success
				 */
   ...
   closeCls(event){
	...
	searchpartner=(event)=>{
		...
		onChitOk = () => {
			...
			this.buyReqManagementService.itemsCdCheck(this.gridParamModel).subscribe(
								(res: any) => {
										this.items=res;
		...
		itemCdEvent = (data) =>{
			...
			console.log(data.newValue)
			this.buyReqManagementService.itemDetail(this.detailParamModel).subscribe(
				(res: any) => {
						this.itemDetail=res;
		...
		onChitSearchOk = () => {
			...
			this.buyReqManagementService.chitSearchResult(this.chitSearchModel).subscribe(
				(res: any) => {
						...
						this.paramModel.orderDt=this.chitResultModel.orderDt;
						...
						this.buyReqManagementService.chitSearchResultDetail(this.chitSearchModel).subscribe(
							(res: any) => {
